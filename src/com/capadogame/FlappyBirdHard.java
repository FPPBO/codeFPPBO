...
public class FlappyBirdHard extends JPanel implements ActionListener, MouseListener, KeyListener{
    ...
    public FlappyBirdHard(CardLayoutWindow parent, int areaWidth, int areaHeight, Bird birdType, int target) {
    	...
    	jbReturn.setActionCommand("ReturnHard");
    ...
    public void addColumn(boolean start){
        ...
        if(start){
            columns.add(new Rectangle(areaWidth+width+columns.size()*300, areaHeight - height - 120, width, height));
            columns.add(new Rectangle(areaWidth + width + (columns.size() - 1)*300, 0, width, areaHeight-height-space));
        }
        else{
            columns.add(new Rectangle(columns.get(columns.size()-1).x + 600, areaHeight - height - 120, width, height));
            columns.add(new Rectangle(areaWidth + width + (columns.size() - 1)*300, 0, width, areaHeight-height-space));
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (started && score < target && !gameOver) {
            ...
            if(bird.y + yMotion >= areaHeight - 120){
                bird.y = areaHeight - 120 - bird.height - 30;
            }
    ...
    public void paint(Graphics g){
        ...
        if(!started){ // kalau belum mulai
            g.drawString("Click to start!!", 40, areaHeight/2);
        }
        if(gameOver && score < target){ // kalau sudah gameover
            g.drawString("Game Over!", 75, areaHeight/2);
        }
